...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // Exact matching keys from your frontend Quote and Contact forms
  const { name, email, service, urgency, context, pageestimate, message, phone } = req.body;
  ...
  try {
    ...
    return res.status(500).json({ error: 'Internal server failed to send email' });
  }
}